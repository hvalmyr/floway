...
// Reacted to by main.css (html.tree-enabled body) so the page background
// goes transparent and lets the fixed 3D canvas show through — see
// useTreeMode.ts.
useHead({
  htmlAttrs: { class: computed(() => (treeEnabled.value ? "tree-enabled" : "")) },
});
...
// Text-heavy pages (blog, legal documents, the thank-you page, contacts)
// get a closer, larger view of the ambient branch instead of the normal
// small-in-the-distance framing — see AmbientTreeBackground's `closeup`
// prop. No longer literally "every page using UiGlassPage.vue" now that
// contacts is in this list too (it uses its own per-section glass panels,
// not that component) — the name tracks the visual treatment, not the
// component. Persists across client-side navigation without remounting the
// 3D scene (it's a layout-level singleton, outside <slot/>), so this just
...
<template>
  <div class="flex min-h-screen flex-col">
    ...
    <LazyAmbientTreeBackground v-if="treeEnabled && showBackground" :closeup="isCloseupPage" />
    <AppHeader />
    <main class="flex-1">
      <slot />
    </main>
