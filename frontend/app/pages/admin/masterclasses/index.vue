...
const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<Masterclass>('/api/v1/masterclasses')
...
<template>
  <div>
    ...
    <form class="mt-6 grid gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2" @submit.prevent="onSubmit">
      ...
      <input v-model="form.duration" type="text" placeholder="Длительность" class="rounded border border-gray-300 px-3 py-2">
      <input v-model="form.coverImage" type="text" placeholder="URL обложки" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2">

      <textarea v-model="form.shortDescription" placeholder="Краткое описание" rows="2" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
      <textarea v-model="form.fullDescription" placeholder="Полное описание" rows="3" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
      <textarea v-model="form.endingText" placeholder="Завершающий текст" rows="2" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />

      <input v-model.number="form.priceGroup" type="number" placeholder="Цена (группа)" class="rounded border border-gray-300 px-3 py-2">
      <input v-model.number="form.priceIndividual" type="number" placeholder="Цена (индивидуально)" class="rounded border border-gray-300 px-3 py-2">
      <textarea v-model="form.priceDescription" placeholder="Описание цены" rows="2" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
    ...
    <table v-else class="mt-6 w-full border-collapse overflow-hidden rounded border border-gray-200 bg-white text-sm">
      ...
      <tbody>
        <tr v-for="masterclass in items" :key="masterclass.id" class="border-b border-gray-100 last:border-0">
          <td class="px-4 py-2">{{ masterclass.title }}</td>
          <td class="px-4 py-2">{{ masterclass.slug }}</td>
          <td class="px-4 py-2">{{ masterclass.priceGroup }} / {{ masterclass.priceIndividual }}</td>
          <td class="px-4 py-2">{{ masterclass.status === 'active' ? 'Активен' : 'В архиве' }}</td>
          <td class="flex gap-3 px-4 py-2 text-right">
            <button class="text-[var(--color-primary)] hover:underline" @click="startEdit(masterclass)">Редактировать</button>
            <button class="text-red-600 hover:underline" @click="onDelete(masterclass.id)">Удалить</button>
          </td>
        </tr>
