...
const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<Lesson>(`/api/v1/course-blocks/${blockId}/lessons`)
...
<template>
  <div>
    ...
    <form class="mt-6 grid gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2" @submit.prevent="onSubmit">
      <input v-model.number="form.number" type="number" placeholder="Номер урока" required class="rounded border border-gray-300 px-3 py-2">
      <input v-model="form.title" type="text" placeholder="Заголовок" required class="rounded border border-gray-300 px-3 py-2">
      <textarea v-model="form.topics" placeholder="Темы" rows="3" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
      <textarea v-model="form.outcomes" placeholder="Результаты" rows="3" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
      <input v-model.number="form.durationHours" type="number" placeholder="Продолжительность (часы)" class="rounded border border-gray-300 px-3 py-2">
      ...
      <tbody>
        <tr v-for="lesson in items" :key="lesson.id" class="border-b border-gray-100 last:border-0">
          ...
          <td class="flex gap-3 px-4 py-2 text-right">
            <button class="text-[var(--color-primary)] hover:underline" @click="startEdit(lesson)">Редактировать</button>
            <button class="text-red-600 hover:underline" @click="onDelete(lesson.id)">Удалить</button>
          </td>
        </tr>
