...
const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<FAQItem>('/api/v1/faq')
...
<template>
  <div>
    ...
    <form class="mt-6 grid gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2" @submit.prevent="onSubmit">
      <textarea v-model="form.question" placeholder="Вопрос" rows="2" required class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
      <textarea v-model="form.answer" placeholder="Ответ" rows="3" required class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
    ...
    <table v-else class="mt-6 w-full border-collapse overflow-hidden rounded border border-gray-200 bg-white text-sm">
      ...
      <tbody>
        <tr v-for="faqItem in items" :key="faqItem.id" class="border-b border-gray-100 last:border-0">
          <td class="px-4 py-2">{{ faqItem.question }}</td>
          <td class="px-4 py-2">{{ faqItem.sortOrder }}</td>
          <td class="flex gap-3 px-4 py-2 text-right">
            <button class="text-[var(--color-primary)] hover:underline" @click="startEdit(faqItem)">Редактировать</button>
            <button class="text-red-600 hover:underline" @click="onDelete(faqItem.id)">Удалить</button>
          </td>
        </tr>
