...
const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<Course>('/api/v1/courses')
...
async function onSubmit() {
  ...
  try {
    const payload = {
      ...
      gallery: form.value.galleryText.split(',').map((s) => s.trim()).filter(Boolean),
...
<template>
  <div>
    ...
    <form class="mt-6 grid gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2" @submit.prevent="onSubmit">
      <input v-model="form.title" type="text" placeholder="Заголовок" required class="rounded border border-gray-300 px-3 py-2">
      ...
      <textarea v-model="form.fullDescription" placeholder="Полное описание" rows="4" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
      <input v-model="form.coverImage" type="text" placeholder="URL обложки" class="rounded border border-gray-300 px-3 py-2">
      <input v-model="form.galleryText" type="text" placeholder="Галерея: URL через запятую" class="rounded border border-gray-300 px-3 py-2">
      <select v-model="form.status" class="rounded border border-gray-300 px-3 py-2">
        <option value="active">Активен</option>
        <option value="archived">В архиве</option>
      </select>
      <input v-model.number="form.sortOrder" type="number" placeholder="Порядок сортировки" class="rounded border border-gray-300 px-3 py-2">
      ...
      <tbody>
        <tr v-for="course in items" :key="course.id" class="border-b border-gray-100 last:border-0">
          ...
          <td class="flex gap-3 px-4 py-2 text-right">
            <NuxtLink :to="`/admin/courses/${course.id}/blocks`" class="text-[var(--color-primary)] hover:underline">Блоки курса</NuxtLink>
            <button class="text-[var(--color-primary)] hover:underline" @click="startEdit(course)">Редактировать</button>
            <button class="text-red-600 hover:underline" @click="onDelete(course.id)">Удалить</button>
          </td>
        </tr>
        <tr v-if="items.length === 0">
          <td colspan="4" class="px-4 py-6 text-center text-[var(--color-text-muted)]">Пока пусто</td>
        </tr>
      </tbody>
    </table>
  </div>
