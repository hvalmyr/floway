...
const emptyForm = (): Omit<CourseFAQItem, "id" | "courseId"> => ({
  question: "",
  answer: "",
  sortOrder: 0,
});

const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<CourseFAQItem>(
  `/api/v1/courses/${courseId}/faq-items`,
);
...
function startEdit(item: CourseFAQItem) {
...
<template>
  <div>
    ...
    >
    <h1 class="mt-2 text-2xl font-semibold">FAQ курса #{{ courseId }}</h1>
    <p class="mt-1 text-sm text-[var(--color-text-muted)]">
      Заголовок и текст перед вопросами, а также видимость всего блока настраиваются в карточке
      курса (кнопка «Редактировать» в списке курсов секции).
    </p>
