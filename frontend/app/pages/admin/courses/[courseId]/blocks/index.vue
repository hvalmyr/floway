...
const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<CourseBlock>(`/api/v1/courses/${courseId}/blocks`)
...
<template>
  <div>
    <NuxtLink to="/admin/courses" class="text-sm text-[var(--color-primary)] hover:underline">← К списку курсов</NuxtLink>
    ...
    <form class="mt-6 grid gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2" @submit.prevent="onSubmit">
      ...
      <input v-model.number="form.lessonsCount" type="number" placeholder="Количество уроков" class="rounded border border-gray-300 px-3 py-2">
      <input v-model.number="form.hours" type="number" placeholder="Часы" class="rounded border border-gray-300 px-3 py-2">
      <input v-model.number="form.price" type="number" placeholder="Цена" class="rounded border border-gray-300 px-3 py-2">
      ...
      <tbody>
        <tr v-for="block in items" :key="block.id" class="border-b border-gray-100 last:border-0">
          ...
          <td class="flex gap-3 px-4 py-2 text-right">
            <NuxtLink :to="`/admin/course-blocks/${block.id}/lessons`" class="text-[var(--color-primary)] hover:underline">Уроки</NuxtLink>
            <button class="text-[var(--color-primary)] hover:underline" @click="startEdit(block)">Редактировать</button>
            <button class="text-red-600 hover:underline" @click="onDelete(block.id)">Удалить</button>
          </td>
        </tr>
