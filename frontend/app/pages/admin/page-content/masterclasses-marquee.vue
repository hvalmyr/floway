...
const { items, loading, error, savingKey, savedKey, fetchAll, save, onIconSelected } =
  useAdminPageContent(["masterclasses_marquee_text", "masterclasses_marquee_icon"]);
...
<template>
  <div>
    <NuxtLink to="/admin" class="text-sm text-[var(--color-primary)] hover:underline"
      >← К дашборду</NuxtLink
    >
    <h1 class="mt-2 text-2xl font-semibold">Бегущая строка мастер-классов</h1>
    <p class="mt-2 text-sm text-[var(--color-text-muted)]">
      Синяя полоса с текстом и иконкой на странице мастер-классов, над каталогом.
    </p>
