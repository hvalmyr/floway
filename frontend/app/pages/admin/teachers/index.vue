...
const emptyForm = (): Omit<Teacher, 'id'> => ({ name: '', photo: '', description: '', sortOrder: 0 })

const { items, loading, error, fetchAll, create, update, remove } = useAdminResource<Teacher>('/api/v1/teachers')
...
<template>
  <div>
    ...
    <form class="mt-6 grid gap-3 rounded border border-gray-200 bg-white p-4 sm:grid-cols-2" @submit.prevent="onSubmit">
      <input v-model="form.name" type="text" placeholder="Имя" required class="rounded border border-gray-300 px-3 py-2 sm:col-span-2">
      <input v-model="form.photo" type="text" placeholder="URL фото" class="rounded border border-gray-300 px-3 py-2">
      ...
      <textarea v-model="form.description" placeholder="Описание" rows="3" class="rounded border border-gray-300 px-3 py-2 sm:col-span-2" />
    ...
    <table v-else class="mt-6 w-full border-collapse overflow-hidden rounded border border-gray-200 bg-white text-sm">
      ...
      <tbody>
        <tr v-for="teacher in items" :key="teacher.id" class="border-b border-gray-100 last:border-0">
          <td class="px-4 py-2">{{ teacher.name }}</td>
          <td class="px-4 py-2">{{ teacher.sortOrder }}</td>
          <td class="flex gap-3 px-4 py-2 text-right">
            <button class="text-[var(--color-primary)] hover:underline" @click="startEdit(teacher)">Редактировать</button>
            <button class="text-red-600 hover:underline" @click="onDelete(teacher.id)">Удалить</button>
          </td>
        </tr>
        <tr v-if="items.length === 0">
          <td colspan="3" class="px-4 py-6 text-center text-[var(--color-text-muted)]">Пока пусто</td>
        </tr>
      </tbody>
    </table>
  </div>
