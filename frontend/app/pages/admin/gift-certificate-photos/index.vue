...
interface GiftCertificateCarouselPhoto {
  id: number;
  image: string;
  sortOrder: number;
}

const emptyForm = (): Omit<GiftCertificateCarouselPhoto, "id"> => ({ image: "", sortOrder: 0 });

const { items, loading, error, fetchAll, create, update, remove } =
  useAdminResource<GiftCertificateCarouselPhoto>("/api/v1/gift-certificate-carousel-photos");
...
function startEdit(photo: GiftCertificateCarouselPhoto) {
...
<template>
  <div>
    <NuxtLink to="/admin" class="text-sm text-[var(--color-primary)] hover:underline"
      >← К дашборду</NuxtLink
    >
    <h1 class="mt-2 text-2xl font-semibold">Карусель (страница сертификатов)</h1>
    <p class="mt-2 text-sm text-[var(--color-text-muted)]">
      Вертикальные фото, показанные в слайд-шоу сразу после Hero-блока на странице подарочных
      сертификатов. Порядок сортировки определяет порядок слайдов.
    </p>
