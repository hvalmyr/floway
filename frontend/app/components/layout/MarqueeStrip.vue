...
/**
 * Full-width scrolling promo strip — icon + message repeated in a seamless
 * loop, the whole strip a single link. Shared by GiftCertificateMarquee
 * (homepage) and MasterclassesMarquee (masterclasses page); see those for
 * the content-key wiring.
 *
  ...
  >
    <div ref="trackEl" aria-hidden="true" class="flex w-max items-center gap-48 whitespace-nowrap">
      <span v-for="n in REPEAT_COUNT" :key="n" class="flex shrink-0 items-center gap-12">
        <AppIcon :icon="props.icon" class="size-24 shrink-0 text-white" />
        <span class="font-display text-h4 text-white">{{ props.message }}</span>
      </span>
    </div>
  </NuxtLink>
</template>
