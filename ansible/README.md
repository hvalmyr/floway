...
### Второй сайт (decor, flo-way.ru)

Джоба `deploy-decor` гоняет тот же `playbooks/deploy.yml` с
`-e target_group=floway_decor_prod` на тот же VPS (тот же SSH-ключ/
known_hosts, тот же `ANSIBLE_VAULT_PASSWORD` — секреты репозитория заводить
второй раз не нужно). Она сама себя гейтит: пока в репозитории нет
`inventory/group_vars/floway_decor_prod/vault.yml` (создаётся один раз через
`just vault-edit-decor`, см. `vault.yml.example` рядом), шаги деплоя и
смоук-проверки пропускаются — джоба зелёная, но ничего не катит. Как только
секреты созданы и закоммичены — деплой decor-сайта начинает идти на каждый
пуш в `main` вместе со школьным, независимо от него (`deploy`/`deploy-decor`
— разные concurrency-группы, не блокируют друг друга).

Ещё одно условие для настоящего запуска (уже после того как секреты
заведены) — A/AAAA-запись `flo-way.ru` должна указывать на IP этой VPS,
иначе Caddy не пройдёт ACME и смоук-проверка `https://flo-way.ru/healthz`
упадёт.
