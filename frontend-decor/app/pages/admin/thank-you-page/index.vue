...
import type { ThankYouPage, ThankYouPageFaqItem, ThankYouPagePhoto } from "~/types/api";

// Decor site has exactly one variant ("decor", seeded by migration 00061) —
// no tab switcher needed, unlike the school's four. Blog-link section
// dropped too — decor has no blog.
const VARIANT = "decor";
...
async function load() {
  ...
  loadError.value = "";
  try {
    const data = await api<ThankYouPage>(`/api/v1/thank-you-pages/${VARIANT}`);
    settings.value = data;
...
async function saveSettings() {
  ...
  try {
    await api(`/api/v1/thank-you-pages/${VARIANT}`, {
      method: "PUT",
      body: {
...
const { draggingIndex: draggingPhotoIndex, onPointerDown: onPhotoPointerDown } =
  useAdminDragReorder(photos, (item) =>
    api(`/api/v1/thank-you-pages/${VARIANT}/photos/${item.id}`, {
      method: "PUT",
...
async function addPhoto() {
  ...
  try {
    const created = await api<ThankYouPagePhoto>(`/api/v1/thank-you-pages/${VARIANT}/photos`, {
      method: "POST",
      body: { image: photoForm.value.image, sortOrder: photos.value.length },
    });
...
async function removePhoto(id: number) {
  ...
  await api(`/api/v1/thank-you-pages/${VARIANT}/photos/${id}`, { method: "DELETE" });
...
const { draggingIndex: draggingFaqIndex, onPointerDown: onFaqPointerDown } = useAdminDragReorder(
  faqItems,
  (item) =>
    api(`/api/v1/thank-you-pages/${VARIANT}/faq-items/${item.id}`, {
      method: "PUT",
...
async function addFaqItem() {
  ...
  try {
    const created = await api<ThankYouPageFaqItem>(`/api/v1/thank-you-pages/${VARIANT}/faq-items`, {
      method: "POST",
      body: {
...
async function removeFaqItem(id: number) {
  ...
  await api(`/api/v1/thank-you-pages/${VARIANT}/faq-items/${id}`, { method: "DELETE" });
...
<template>
  <div>
    ...
    <p class="mt-2 text-sm text-[var(--color-text-muted)]">
      Содержимое страницы, на которую попадает посетитель сразу после отправки заявки.
    </p>
    ...
    <template v-else-if="settings">
      ...
      >
        ...
        <h2 class="font-semibold">Мессенджеры</h2>
        <p class="text-sm text-[var(--color-text-muted)]">
          Иконки берутся из общих настроек контактов. Здесь только показываете или скрываете их на
          этой странице.
        </p>
      ...
      <div class="mt-6 rounded border border-gray-200 bg-white p-4">
        <h2 class="font-semibold">Карусель фото</h2>
