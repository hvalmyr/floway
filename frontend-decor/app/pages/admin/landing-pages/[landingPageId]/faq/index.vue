...
const emptyForm = (): Omit<LandingPageFAQItem, "id" | "landingPageId"> => ({
  question: "",
  answer: "",
  sortOrder: 0,
});
...
const { items, loading, error, fetchAll, create, update, remove } =
  useAdminResource<LandingPageFAQItem>(`/api/v1/landing-pages/${landingPageId}/faq-items`);
...
function startEdit(item: LandingPageFAQItem) {
  editingId.value = item.id;
  form.value = { question: item.question, answer: item.answer, sortOrder: item.sortOrder };
}
...
<template>
  <div>
    ...
    >
    <h1 class="mt-2 text-2xl font-semibold">FAQ страницы #{{ landingPageId }}</h1>
    <p class="mt-1 text-sm text-[var(--color-text-muted)]">
      Заголовок и текст перед вопросами, а также видимость всего блока настраиваются в карточке
      страницы (кнопка «Редактировать» в списке посадочных страниц).
    </p>
