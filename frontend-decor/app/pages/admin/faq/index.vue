...
// Flat, unscoped homepage FAQ — decor's per-landing-page FAQ blocks live at
// /admin/landing-pages/{id}/faq instead (mirrors Course's own FAQ on the
// school), so this page doesn't need the masterclasses/gift_certificate
// tabs the school's faq/index.vue has.
interface FAQItem {
  id: number;
  question: string;
  answer: string;
...
function startEdit(faqItem: FAQItem) {
  ...
  form.value = { question: faqItem.question, answer: faqItem.answer, sortOrder: faqItem.sortOrder };
}
...
<template>
  <div>
    <NuxtLink to="/admin" class="text-sm text-[var(--color-primary)] hover:underline"
      >← К дашборду</NuxtLink
    >
    <h1 class="mt-2 text-2xl font-semibold">FAQ главной страницы</h1>
