...
import { contactMethodLabels, leadFormatLabels, sourceLabels } from "~/lib/leadLabels";
...
// Decor site has one request type — no course/masterclass/gift-certificate
// select needed (unlike the school's leads admin, which offers those four).
const CONTACT_METHODS: ContactMethod[] = ["call", "telegram", "whatsapp", "max"];
const SOURCES: LeadSource[] = ["referral", "ads", "internet", "social", "maps"];
const FORMATS: LeadFormat[] = ["season", "event"];
...
async function fetchObjectTypes() {
  objectTypes.value = (await api<ObjectType[]>("/api/v1/object-types")) ?? [];
}

await Promise.all([fetchAll(), fetchTagOptions(), fetchConversion(), fetchObjectTypes()]);

function objectTypeName(id?: number) {
  if (!id) return "";
  return objectTypes.value.find((t) => t.id === id)?.name ?? "";
}

// Manual lead entry — for when someone calls in directly instead of
// submitting the site form.
...
function resetCreateForm() {
  ...
  createForm.objectTypeId = 0;
  createForm.format = "season";
  createError.value = "";
}
  ...
  try {
    await api("/api/v1/leads", {
      ...
      body: {
        ...
        requestType: "decor",
        objectTypeId: createForm.objectTypeId || undefined,
...
<template>
  <div>
    ...
    >
      ...
      <div>
        <label class="text-xs text-[var(--color-text-muted)]">Тип объекта</label>
        <select
          v-model="createForm.objectTypeId"
          class="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm"
        >
      ...
      <div>
        <label class="text-xs text-[var(--color-text-muted)]">Формат</label>
        <select
          v-model="createForm.format"
          class="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm"
        >
          <option v-for="f in FORMATS" :key="f" :value="f">{{ leadFormatLabels[f] }}</option>
        </select>
      </div>
      ...
      >
        <div class="min-w-0 flex-1">
          ...
          <p class="mt-1 text-sm text-[var(--color-text-muted)]">
            ...
            <template v-if="objectTypeName(lead.objectTypeId)">
              · {{ objectTypeName(lead.objectTypeId) }}</template
            >
            <template v-if="lead.format"> · {{ leadFormatLabels[lead.format] }}</template>
