...
- `leads`: публичный `POST /api/v1/leads` (форма с сайта, rate-limit 3/час/IP) + `GET/PATCH .../status/DELETE` для админки.
...
- **Email/Telegram-уведомления при создании лида** — `SMTP_*`/`TELEGRAM_*` в `.env.example` заведены, но заявка сейчас нигде не триггерит отправку; менеджер должен сам заходить в админку смотреть список лидов.
